...
def detect_lock_marker(
    ...
    *,
    ...
    # The hybrid vision maps team 1 to the left outcome column and team 2 to
    # the middle column. Restrict the search to that team's label/icon area so
    # a lock on "Не будет N-го гола" cannot mark a team outcome as blocked.
    if side == 1:
    ...
    for label in range(1, count):
        ...
        # Padlock silhouette: a relatively sparse shackle above a dense body.
...
async def detect_locked_next_goal_sides(
    ...
    *,
    ...
    """Capture the current market canvas and report visually locked team sides."""
    try:
        ...
        # This is an additional safety signal. A detector/capture failure must
        # not destroy the existing hybrid DOM+Canvas reader.
        return {
            "locked_sides": [],
            "markers": {},
...
async def read_next_goal_odds(
    ...
    *,
    ...
    """Preserve the hybrid reader and add a visual MARKET_LOCKED guard."""
    ...
    if locked_sides:
        ...
        raise hybrid_market.MarketNotAvailable(
            f"Рынок следующего гола №{odds.next_goal_number} заблокирован букмекером.",
