...
from .strategy import DEFAULT_STRATEGY_CONFIG, money


DEMO_START_BUDGET = money(sum(DEFAULT_STRATEGY_CONFIG.stakes, Decimal("0")))
...
class DemoBudget:
    ...
    def restore(
        self,
        initial_budget: Decimal | int | float | str,
        current_budget: Decimal | int | float | str,
    ) -> None:
    ...
    def snapshot(self) -> dict[str, float]:
        return {
            ...
            "session_profit": float(self.session_profit),
        }
    ...
    def settle(
        ...
        actual_odds: Decimal | int | float | str = 2,
    ) -> dict[str, Any] | None:
        ...
        gross_return = (
            money(stake_value * odds) if result == "WIN" else Decimal("0.00")
        )
        ...
        return {
            ...
            "budget_after": float(self.current_budget),
            "pnl": float(change),
